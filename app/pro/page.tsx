...
import { NativePriceProvider } from "@/components/pro/NativePriceProvider";
import { PriceLabel } from "@/components/pro/PriceLabel";
import { RestoreButton } from "@/components/pro/RestoreButton";
import { SubscriptionDisclosures } from "@/components/pro/SubscriptionDisclosures";
  ...
  return (
    <NativePriceProvider skus={skus}>
      <main className="container max-w-3xl space-y-8 py-8">
        ...
        {params.canceled && (
          <div className="rounded-2xl border-2 border-cloud-deep bg-cloud p-3 text-sm text-ink/70 dark:border-ink-light/60 dark:bg-ink-mid dark:text-cloud/70">
        ...
        <section className="grid gap-3 md:grid-cols-2">
          {FEATURES.map((f) => (
            <div key={f.title} className="card-pop flex items-start gap-3 p-4">
              ...
              <div>
                <p className="text-sm font-extrabold text-ink dark:text-cloud">{f.title}</p>
                <p className="text-xs text-ink/70 dark:text-cloud/70">{f.body}</p>
              </div>
            </div>
          ))}
        </section>
          ...
          {list.map((p) => {
            ...
            return (
              <Card
                ...
                className={
                  isFeatured
                    ? "relative border-2 border-sky bg-sky/5 p-6 dark:bg-sky/15"
                    : "relative p-6"
                }
              >
                {isFeatured && (
                  <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-sky px-3 py-1 text-[11px] font-extrabold uppercase tracking-wider text-white shadow-pop">
                    Mais escolhido
                  </span>
                )}
                <h3 className="text-lg font-black text-ink dark:text-cloud">{p.name}</h3>
                <p className="mt-1 min-h-[40px] text-xs text-ink/70 dark:text-cloud/70">
                  {p.description}
                </p>
                ...
                <div className="mt-4">
                  <PriceLabel
                    sku={p.sku}
                    fallback={price}
                    className="text-3xl font-black text-ink dark:text-cloud"
                  />
                  {!isLifetime && (
                    <span className="ml-1 text-sm text-ink/60 dark:text-cloud/60">
                ...
                {trialDays > 0 && !isLifetime && (
                  <p className="mt-1 text-xs font-bold text-grass-deep dark:text-grass-soft">
        ...
        {/* Apple Review Guideline 3.1.1 — Restore Purchases must be
            available on the same screen as the purchase action. Rendered
            only on native; web users go through Stripe and don't need it. */}
        <RestoreButton variant="outline" className="mx-auto max-w-sm" />

        {/* Apple Review Guideline 3.1.2 — auto-renewing subscription
            disclosures must appear adjacent to the purchase action. */}
        <SubscriptionDisclosures />

        <p className="pt-2 text-center text-xs text-ink/50 dark:text-cloud/50">
